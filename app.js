...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('import-btn')?.addEventListener('click', async () => {
    ...
    if (!url) {
      showToast('⚠️ Please enter a URL', true);
      return;
    }
    if (!/^https?:\/\//i.test(url)) {
      url = 'https://' + url;
    ...
    try {
      ...
      const twCard = doc.querySelector('meta[name="twitter:card"]')?.getAttribute('content') || 'summary';
      ...
      document.getElementById('og-desc').value = ogDesc;
      document.getElementById('og-image').value = ogImage;
      const typeSelect = document.getElementById('og-type');
      if (Array.from(typeSelect.options).some(o => o.value === ogType)) typeSelect.value = ogType;
      document.getElementById('og-site-name').value = ogSiteName;

      const cardSelect = document.getElementById('tw-card');
      if (Array.from(cardSelect.options).some(o => o.value === twCard)) cardSelect.value = twCard;
      document.getElementById('tw-title').value = twTitle;
      document.getElementById('tw-desc').value = twDesc;
      document.getElementById('tw-image').value = twImage;
      document.getElementById('tw-site').value = twSite;

      // Trigger updates
      ['page-title', 'page-desc', 'page-url', 'og-title', 'og-desc', 'og-image', 'og-site-name'].forEach(id => {
        document.getElementById(id)?.dispatchEvent(new Event('input'));
      });
      document.getElementById('og-type')?.dispatchEvent(new Event('change'));
